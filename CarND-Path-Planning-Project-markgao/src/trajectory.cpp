                                  ...
                                  const vector<double> &map_waypoints_x, const vector<double> &map_waypoints_y, const vector<double> &map_waypoints_s)
{
    ...
    vector<vector<double>> trajectoryXY;
    ...
    // Convert the global coordinate system to the vichle coordinate system
    for(int i=0; i<spine_x.size(); i++)
    {
        double shift_to_car_x = spine_x[i] - ref_x;
        double shift_to_car_y = spine_y[i] - ref_y;
        spine_x[i] = shift_to_car_x*cos(0 - ref_yaw) - shift_to_car_y*sin(0 - ref_yaw);
        spine_y[i] = shift_to_car_x*sin(0 - ref_yaw) + shift_to_car_y*cos(0 - ref_yaw);
    }
    ...
    trajectoryXY.push_back(next_x_vals);
    trajectoryXY.push_back(next_y_vals);
