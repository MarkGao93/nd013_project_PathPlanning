...
int main() {
    ...
    double max_s = 6945.554;    // max s of a single loop
    ...
    double lane = 1;    // 0 for left lane, 1 for middle lane, 2 for right lane
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    */
                    ...
                    int PATH_SIZE = 50;    // 30
                    double max_v = 49.5;    // max vehicle speed
                    double ref_x = car_x;
                    double ref_y = car_y;
                    ...
                    // Get range of the nearest front car and target velocity
                    vector<double> predict_front = prediction_front(sensor_fusion, path_size, lane, future_car_s);

                    // Get range and velocity of the nearest front and rear cars in adjacent lanes
                    ...
                    // Variable ref_vel and lane will be changed in this function
                    // Return bool change_lane, 1 for change lane, 0 for stay in current lane
                    bool change_lane = vehicle(predict_front, predict_left_right, ref_vel, lane, max_v);

                    // Generate trajectory points
                    vector<vector<double>> traject = trajectory(previous_path_x, previous_path_y,
                                                                ref_x, ref_y, ref_yaw,
                                                                change_lane, PATH_SIZE, lane, ref_vel, car_s,
                                                                map_waypoints_x, map_waypoints_y, map_waypoints_s);

                    // Send trajectory points to simulator
